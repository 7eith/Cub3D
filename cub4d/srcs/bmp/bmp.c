...
/*   Updated: 2020/10/24 02:49:38 by amonteli         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
void		init_bmp_data(t_game *vars)
{
	...
	vars->bmp->width_in_bytes = ((vars->bmp->width *
	vars->bmp->bytes_count + 31) / 32) * 4;
...
void		save_bmp(t_game *vars)
{
	...
	{
		write(vars->bmp->fd, vars->img_data
		+ (height_to_write * vars->bmp->width), vars->bmp->width * 4);
		height_to_write--;
	}
	close(vars->bmp->fd);
	clean_exit(vars);
